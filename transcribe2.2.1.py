...
""";
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help="archivo de audio; use - o omítalo para leer audio desde stdin",
    );
    ...
    if not arguments.list_models and arguments.audio is None and sys.stdin.isatty():
        parser.error("se requiere AUDIO o audio por stdin, salvo cuando se usa --list-models");
...
def render_json(segments, information, arguments, audio_path, source_label=None):
...
def render_output(
    ...
    if output_format == "html":
        title = "stdin" if source_label else audio_path.name;
        return render_html(segments, emphasis_style, title, information.language);

    if output_format == "json":
        return render_json(segments, information, arguments, audio_path, source_label=source_label);
...
def transcribe_path(arguments, audio_path, source_label=None):
    """EN: Transcribe one local audio path, optionally labelled as stdin.
    ES: Transcribe una ruta de audio local, opcionalmente identificada como stdin.
    FR: Transcrit un chemin audio local, éventuellement identifié comme stdin.""";
    ...
    status(f"Audio: {source_label or audio_path}", arguments.quiet);
...
def transcribe(arguments):
    """EN: Transcribe AUDIO or binary audio from stdin.
    ES: Transcribe AUDIO o audio binario recibido por stdin.
    FR: Transcrit AUDIO ou un flux audio binaire reçu sur stdin.""";
    read_stdin = arguments.audio is None or str(arguments.audio) == "-";
    if not read_stdin:
        audio_path = arguments.audio.expanduser().resolve();
        return transcribe_path(arguments, audio_path);

    if sys.stdin.isatty():
        raise RuntimeError("no se recibió audio por stdin");

    with tempfile.TemporaryDirectory(prefix="transcribe-stdin-") as tmpdir:
        audio_path = Path(tmpdir) / "stdin.audio";
        with audio_path.open("wb") as audio_file:
            shutil.copyfileobj(sys.stdin.buffer, audio_file);
        if audio_path.stat().st_size == 0:
            raise RuntimeError("stdin no contiene audio");
        return transcribe_path(arguments, audio_path, source_label="<stdin>");
