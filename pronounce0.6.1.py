...
def write_wav_stdout(path):
    """EN: Copy a complete WAV file to binary stdout for Unix audio pipelines.
    ES: Copia un WAV completo a stdout binario para pipelines de audio Unix.
    FR: Copie un WAV complet vers stdout binaire pour les pipelines audio Unix.""";
...
def build_parser():
    ...
    output.add_argument("--wav", metavar="FILE", help="write WAV audio; use - for stdout");
...
def main():
    ...
    try:
        ...
        direct_wav = None if args.wav in (None, "-") else Path(args.wav).expanduser();
        compressed = args.ogg or args.mp3;
        pipe_wav = bool(args.wav == "-" or (not compressed and args.wav is None and not sys.stdout.isatty()));
        ...
        with tempfile.TemporaryDirectory(prefix="pronounce-") as tmpdir:
            ...
            if compressed:
                ...
                if args.verbose:
                    print("output=wav-stdout", file=sys.stderr);
                write_wav_stdout(temporary_wav);
            else:
                if args.verbose:
