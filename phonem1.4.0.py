...
EN: Logical study profiles are kept separate from installed eSpeak voices. Missing
    dialect voices such as fr-ca, en-ca and es-uy are explicit approximations, never
...
def normalize_language(language):
    """EN: Normalize a locale name. ES: Normaliza una locale. FR: Normalise le nom d'une locale."""
...
def detect_language(text):
    ...
    if best_score == 0:
        raise ValueError("language is ambiguous; specify -l es-uy, -l fr-ca, -l en-ca, etc.");
...
def language_family(logical_language):
    """EN: Return EN/ES/FR family. ES: Devuelve familia EN/ES/FR. FR: Renvoie la famille EN/ES/FR."""
    profile = LANGUAGE_PROFILES.get(logical_language, {});
    if profile.get("family"):
        return profile["family"];
    family = logical_language.split("-", 1)[0];
    if family in {"en", "es", "fr"}:
        return family;
    return None;
...
def print_languages(supported, config):
    ...
    print("Alias            eSpeak backend     Mode           Description");
    print("---------------  -----------------  -------------  ------------------------------------------------------------");
    for alias, profile in LANGUAGE_PROFILES.items():
        try:
            ...
            mode = "-";
        print(f"{alias:<15}  {backend_language:<17}  {mode:<13}  {profile['label']}");
