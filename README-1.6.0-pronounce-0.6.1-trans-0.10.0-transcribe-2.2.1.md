...
Current tools: **phonem.py 1.6.0**, **pronounce.py 0.6.1**, **trans.py 0.10.0**, **transcribe.py 2.2.1**.
...
The recommended installation prepares a private virtual environment, the four commands, Piper defaults and the faster-whisper `small` model:

```bash
./install.sh
```

Use `./install.sh --minimal` for code/dependencies only, or `./install.sh --update` after unpacking a newer release. The primary target is GNU/Linux/POSIX with Python 3.9 or newer. Manual installation remains possible:
...
Audio itself is pipeable too. When `pronounce` has no explicit output option and stdout is not a terminal, it writes a WAV stream instead of opening `ffplay`. `transcribe` accepts that binary stream when `AUDIO` is omitted (or is `-`):

```bash
phonem "I am a good lawyer" -l en-ca | \
    pronounce -l en-ca | \
    transcribe -l en-ca --model medium
```

Use `pronounce --wav -` when explicit binary-stdout behaviour is preferred.
...
La instalación recomendada prepara un entorno virtual privado, los cuatro comandos, las voces Piper predeterminadas y el modelo `small` de faster-whisper:

```bash
./install.sh
```

`./install.sh --minimal` instala sólo código/dependencias y `./install.sh --update` actualiza una instalación al descomprimir una versión nueva. El objetivo principal es GNU/Linux/POSIX con Python 3.9 o posterior. La instalación manual sigue disponible:
...
El audio también se puede encadenar. Si `pronounce` no recibe una opción de salida y stdout no es una terminal, escribe un WAV por el pipe en vez de abrir `ffplay`. `transcribe` consume ese audio binario cuando se omite `AUDIO` (o se usa `-`):
...
`pronounce --wav -` fuerza de forma explícita la salida WAV por stdout.

## Referencia detallada

### Transcripción

`transcribe.py 2.2.1` conserva las funciones de la 2.1.1 y adopta el contrato común:
...
`pronounce.py 0.6.1` usa **schema 2**. El cambio elimina `en`, `es` y `fr` como perfiles JSON independientes: pasan a ser alias auténticos de `en-ca`, `es-uy` y `fr-fr`.
...
L'installation recommandée prépare un environnement virtuel privé, les quatre commandes, les voix Piper par défaut et le modèle `small` de faster-whisper :

```bash
./install.sh
```

`./install.sh --minimal` installe seulement le code et les dépendances ; `./install.sh --update` met à jour une installation après extraction d'une nouvelle version. Cible principale : GNU/Linux/POSIX, Python 3.9 ou plus récent. L'installation manuelle reste disponible :
...
Le flux audio lui-même peut également passer par un pipe. Sans option de sortie explicite, `pronounce` écrit un WAV sur stdout lorsque stdout n'est pas un terminal, au lieu d'ouvrir `ffplay`. `transcribe` lit ce flux binaire lorsque `AUDIO` est omis (ou vaut `-`) :
...
`pronounce --wav -` force explicitement la sortie WAV sur stdout.

## Référence détaillée

### Transcription

`transcribe.py 2.2.1` conserve les fonctions de 2.1.1 et adopte :
...
Le schéma 2 de `pronounce.py 0.6.1` transforme `en`, `es` et `fr` en vrais alias au lieu de profils JSON séparés. Une mise à jour depuis le schéma 1 crée d'abord une sauvegarde, migre les entrées canoniques, supprime les doublons d'alias devenus obsolètes et conserve les réglages canoniques/personnalisés.
