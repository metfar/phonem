...
SUPPORTED_LOGICAL_LANGUAGES = (
    "en", "en-ca", "en-us", "en-gb", "en-rp", "en-lancashire", "en-nyc",
    "es", "es-es", "es-uy",
...
PROFILE_DEFAULTS = {
    "en": {"voice": "en_US-lessac-high", "length_scale": 1.0, "volume": 1.0, "extra_pauses": False, "validation": "starting-point"},
    ...
    "es": {"voice": "es_ES-davefx-medium", "length_scale": 1.0, "volume": 1.0, "extra_pauses": False, "validation": "starting-point"},
    ...
    "fr": {"voice": "fr_FR-siwis-medium", "length_scale": 1.0, "volume": 1.0, "extra_pauses": False, "validation": "starting-point"},
...
def profile_locales(profile):
    ...
    return PIPER_PROFILE_LOCALES.get(profile, ());
...
def print_logical_languages(config):
    ...
    print(f"{'PROFILE':15} {'PIPER LOCALE(S)':18} {'DEFAULT VOICE':38} {'LEN':5} {'VOL':5} {'PAUSE':6} STATUS");
    for profile in SUPPORTED_LOGICAL_LANGUAGES:
        ...
        print(f"{profile:15} {locales:18} {voice[:38]:38} {length_scale:<5g} {volume:<5g} {pauses:6} {validation}");
...
def _voice_matches(key, info, selector):
    ...
    if selector in PIPER_PROFILE_LOCALES:
        return code in profile_locales(selector);
...
def _model_matches(row, selector):
    ...
    if not selector or selector == "*":
        return True;
    if selector in PIPER_PROFILE_LOCALES:
        return row.get("locale", "") in profile_locales(selector);
    needle = selector.lower();
...
def resolve_model(args, config):
    ...
    language = args.language or config.get("default_language") or "";
    requested = "";
    source = "";
    if args.model:
...
def build_parser():
    ...
    parser.add_argument("-l", "--language", choices=SUPPORTED_LOGICAL_LANGUAGES, help="logical language key used to choose a configured voice");
